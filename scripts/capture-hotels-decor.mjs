// Capture the hotels Solutions dark cap at multiple viewport widths.
// Uses a full-page screenshot then crops to the cap rect computed
// from element measurements. Avoids zoom-vs-scrollIntoView quirks.
//
// Output: .tmp/hotels-decor-<width>.png
...
for (const width of WIDTHS) {
  ...
  const measure = await page.evaluate(() => {
    ...
    if (!el) return null;
    // Get document-relative position by computing offsetTop walking up
    let docTop = 0;
    let n = el;
  ...
  const fullPath = path.resolve(`.tmp/hotels-decor-${width}-full.png`);
  await page.screenshot({ path: fullPath, fullPage: true, type: "png" });

  // Crop the full-page PNG to the cap rect (scaled by zoom factor)
  ...
  const cropPath = path.resolve(`.tmp/hotels-decor-${width}.png`);
