...
export function AboutUs() {
  return (
    <section className="py-12 sm:py-20 lg:py-[120px]">
      <div className="overflow-hidden bg-bg-subtle">
        <div className="px-6 pt-12 sm:px-12 sm:pt-16 lg-pad-x lg:pt-[120px]">
        ...
        <div className="flex flex-col gap-12 pt-12 sm:gap-16 sm:pt-16 lg:flex-row lg:min-h-[742px] lg:gap-20 lg:pt-[78px]">
          ...
          <div className="relative border-t border-stroke-default px-6 py-10 sm:px-12 sm:py-12 lg:flex-[1_1_704px] lg:rounded-tl-[48px] lg:border-l lg:border-t lg:px-0 lg:py-12 lg:pl-20 lg:pr-[var(--lg-pad-x)]">
            {/* Left extension — continues border-l from facts col
                bottom-left straight down to where the orange L-shape
                starts. Figma shows this short ~80 px segment between
                Frame 232's bottom and the Shape's top-left corner.
                left:-1 aligns the segment with border-l (the absolute
                child's containing block is the PADDING box, so left:0
                would place the segment 1 px to the right of border-l
                and cause a visible jog). */}
            <span
              aria-hidden
              className="absolute top-full hidden h-[80px] w-px bg-neutral-900 lg:block"
              style={{ left: -1 }}
            />
            {/* Orange dots — Figma Frame 232 has 4 polygons riveted along
                the top + left edges (NOT at the corners). Positions are
                expressed as percentages of the 704×742 card so they
                scale cleanly on viewports smaller than 1440. Centres
                line up with the polygon's centre points in Figma:
                  • top edge: x=141 + 4 = 145 → 20.6% from left
                  • top edge: x=593 + 4 = 597 → 84.8% from left
                  • left edge: y=331 + 4 = 335 → 45.1% from top
                  • left edge: y=588 + 4 = 592 → 79.8% from top
            */}
            <span
              aria-hidden
              className="absolute hidden size-2 -translate-x-1/2 rounded-full bg-brand lg:block"
              style={{ top: -4, left: "20.6%" }}
            />
            <span
              aria-hidden
              className="absolute hidden size-2 -translate-x-1/2 rounded-full bg-brand lg:block"
              style={{ top: -4, left: "84.8%" }}
            />
            <span
              aria-hidden
              className="absolute hidden size-2 -translate-y-1/2 rounded-full bg-brand lg:block"
              style={{ left: -4, top: "45.1%" }}
            />
            <span
              aria-hidden
              className="absolute hidden size-2 -translate-y-1/2 rounded-full bg-brand lg:block"
              style={{ left: -4, top: "79.8%" }}
            />
                ...
                return (
                  <Reveal as="li" key={i} delay={i * 90} direction="left">
                    <div className="group flex items-center gap-4 sm:gap-6">
                      <span
                        aria-hidden
                        className="w-[64px] shrink-0 text-h1 text-neutral-900 transition-colors duration-300 group-hover:text-brand sm:w-[95px]"
                      >
                        {String(i + 1).padStart(2, "0")}.
                      </span>
                      <p className="flex-1 text-body-lg text-neutral-900 transition-transform duration-300 group-hover:translate-x-1">
                        {first}
                        {rest.length > 0 && (
                          <>
                            <br aria-hidden />
        ...
        {/* Bottom area — INSIDE the grey card.
        ...
        <div className="mt-10 px-6 pb-10 sm:mt-12 sm:px-12 sm:pb-12 lg:mt-[80px] lg-pad-x lg:pb-[80px]">
          <div className="relative xl:aspect-[1180/445]">
            <div className="flex flex-col items-stretch gap-4 sm:gap-6 lg:flex-row lg:items-start lg:gap-[1.356%] xl:h-full">
              <div
                className="group relative h-[200px] w-full shrink-0 overflow-hidden rounded-[32px] sm:h-[237px] sm:rounded-[48px] lg:aspect-square lg:h-auto lg:w-[clamp(140px,20.085%,237px)] lg:rounded-[48px]"
              ...
              >
                <img
                  ...
                  loading="lazy"
                  decoding="async"
                  className="absolute h-[130%] w-[129%] max-w-none object-cover transition-transform duration-700 ease-out group-hover:scale-105"
            ...
            <ul className="mt-8 grid grid-cols-3 gap-x-6 gap-y-2 sm:mt-10 sm:gap-x-10 lg:mt-12 lg:gap-x-12 lg:pr-[8%] xl:absolute xl:bottom-0 xl:left-0 xl:right-[46%] xl:mt-0 xl:gap-x-[clamp(20px,3vw,48px)] xl:pr-0">
              {STATS.map((s, i) => (
                <Reveal
                  key={s.label}
                  as="li"
                ...
                >
                  <p className="whitespace-nowrap text-[44px] font-semibold leading-[1.03] tracking-[-1px] text-neutral-900 sm:text-[60px] sm:tracking-[-1.3px] lg:text-[clamp(56px,5vw,72px)] lg:tracking-[-1.5px]">
                    <AnimatedNumber value={s.value} />
                    <span className="text-brand">{s.suffix}</span>
                  </p>
                  <p className="max-w-[200px] text-body-sm leading-snug text-neutral-500">
                    {s.label}
                  </p>
                </Reveal>
              ))}
